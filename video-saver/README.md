...
本地运行的视频 / 图文提取下载站：粘贴分享链接 → 解析 → 预览 → 一键下载无水印视频，图文自动打包为 ZIP；支持图文+视频混排作品（动态图文）同时提取图片与视频。
...
- **抖音**：专用解析器（任意链接规整为作品 ID → 移动端分享页内嵌数据 → 无水印直链），支持视频、图文，以及图文+视频混排（第一张为图片、第二张为视频的动态图文）；解析失败自动降级 yt-dlp
...
- 图片代理接口（`/api/proxy`）带 SSRF 防护：仅放行解析到公网 IP 的 http/https 地址，并对重定向逐跳校验、校验请求来源，防止恶意网页跨站触发访问内网。
- 仅用于个人合法内容存档，请遵守各平台服务条款与版权规定。

## 版本

当前版本 0.2，更新记录见 `CHANGELOG.md`。
